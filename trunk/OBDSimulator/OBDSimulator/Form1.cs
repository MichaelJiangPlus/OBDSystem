...
namespace OBDSimulator
{
    public partial class Form1 : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (isAuto)
            {
                MessageBox.Show("已开启自动模式，请暂停后尝试");
                return;
            }
        ...
        static void Thread_param(Object arg)
        {
            string[] strUp = { "AT+SDS041:UP", "AT+SDS048:UP", "AT+SDS049:UP", "AT+SDS051:UP", "AT+SDS098:UP", "AT+SDS149:UP" };
            string[] strDown = { "AT+SDS041:DOWN", "AT+SDS048:DOWN", "AT+SDS049:DOWN", "AT+SDS051:DOWN", "AT+SDS098:DOWN", "AT+SDS149:DOWN" };
            MySerialPort mycom = (MySerialPort)arg;
            while (true)
            {
                for (int temp = 0; temp < 6; temp++)
                {
                    mycom.MySerialPortSend(strUp[0]);
                    Thread.Sleep(10);

                    mycom.MySerialPortSend(strUp[1]);
                    Thread.Sleep(10);

                    mycom.MySerialPortSend(strUp[2]);
                    Thread.Sleep(10);

                    mycom.MySerialPortSend(strUp[3]);
                    Thread.Sleep(10);

                    mycom.MySerialPortSend(strUp[4]);
                    Thread.Sleep(10);

                    mycom.MySerialPortSend(strUp[5]);
                    Thread.Sleep(100);
                }

                for (int temp = 0; temp < 6; temp++)
                {
                    mycom.MySerialPortSend(strDown[0]);
                    Thread.Sleep(10);

                    mycom.MySerialPortSend(strDown[1]);
                    Thread.Sleep(10);

                    mycom.MySerialPortSend(strDown[2]);
                    Thread.Sleep(10);

                    mycom.MySerialPortSend(strDown[3]);
                    Thread.Sleep(10);

                    mycom.MySerialPortSend(strDown[4]);
                    Thread.Sleep(10);

                    mycom.MySerialPortSend(strDown[5]);
                    Thread.Sleep(100);
                }
            }
        }
        ...
        {
            // string[] str = { "AT+SDS048:UP" , "AT+SDS049:DOWN" };
            ...
            {
                #region Thread无参数举例
                OBDSystemThread = new Thread(Thread_param);
                OBDSystemThread.Start(mycom);
                #endregion
                isAuto = true;
            }
        ...
        {
            isAuto = false;
            if(OBDSystemThread != null){
                OBDSystemThread.Abort();
                OBDSystemThread = null;
            }
        }
    }
}